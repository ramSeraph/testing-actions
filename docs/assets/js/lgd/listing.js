...
import { fileSize, getDateParts, monthCompare, getArchiveList, getObjectLink } from './common';

const displayResults = (sizeMap) => {
    var archivesDiv = document.getElementById('archive_list');
    var objInfos = [];

    for (const dateStr in sizeMap) {
        var objInfo = getDateParts(dateStr);
        objInfo['url'] = getObjectLink(`${dateStr}.zip`);
