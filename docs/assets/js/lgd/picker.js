import '/assets/css/lgd/flatpickr_dark_custom.styl';
import '/assets/css/lgd/status.css';

import flatpickr from 'flatpickr';

import { fileSize, getDateParts, getObjectLink, getArchiveList } from './common';
...
function showLink(date, sizeMap, statusSetter) {
    ...
    const name = dateStr + '.zip';
    const size = fileSize(sizeMap[dateStr]);
    const link = getObjectLink(name);
    statusSetter(`Archive: <a href="${link}">${name}</a> ${size}`, false);
}
...
window.onload = (event) => {
    ...
    const setStatus = (msg, error, selected) => {
    ...
    const fillDatePicker = (data, error) => {
        if (error === true) {
            setStatus(data, true);
            return;
        }
