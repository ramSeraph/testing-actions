...
function move_release {
    ...
    export from_id=$1
    export to=$2
    gh api --method PATCH -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" /repos/${GITHUB_REPOSITORY}/releases/$from_id -f tag_name="$to" -f name="$to" -F make_latest=false 2>>$err_file
}
